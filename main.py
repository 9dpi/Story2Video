...
<div class="card">
  ...
  <div class="sgrid">
    ...
    <div><label>Max Words</label><input type="number" id="maxWords" value="10" min="1" max="50"></div>
  </div>
...
['fontSize','vPos','maxWords','textAlign','bgColorText','textColorText','hlColorText','textOpacity','hlOpacity'].forEach(id=>{
...
function S(){return{
  ...
  mwl:parseInt($('maxWords').value)||10,
...
function drawFrame(time,canvas,settings){
  ...
  if(sub){
    ...
    const lines=wrap(ctx,sub.text,mw,s.mwl),lh=s.fs*1.35,th=lines.length*lh,sy2=y-th/2+lh/2;
...
function wrap(ctx,t,mw,mwl){
  const w=t.split(/\s+/),ls=[];let c='',wc=0;
  w.forEach(x=>{
    const tt=c?c+' '+x:x;
    const curW=ctx.measureText(tt).width;
    if((curW>mw || (mwl && wc>=mwl)) && c){
      ls.push(c);c=x;wc=1;
    }else{
      c=tt;wc++;
    }
